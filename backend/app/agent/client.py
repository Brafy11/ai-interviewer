...
- reading the key from APP_ANTHROPIC_API_KEY (never ANTHROPIC_API_KEY);
...
def _get_client():
    ...
    if _client is None:
        ...
        # Hard rule: read APP_ANTHROPIC_API_KEY explicitly, never ANTHROPIC_API_KEY,
        # so the key can't be picked up silently by other tooling.
        _client = Anthropic(api_key=os.environ["APP_ANTHROPIC_API_KEY"])
    return _client
