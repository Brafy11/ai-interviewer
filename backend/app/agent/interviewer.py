...
INTERVIEW_SYSTEM = (
    ...
    "Rules:\n"
    f"- The interview always asks at least {MIN_QUESTIONS} questions and never more "
    f"than {MAX_QUESTIONS}. `question` must therefore ALWAYS be a genuine, answerable "
    "interview question — never a closing statement, thank-you, or farewell. Even "
    "when you set should_end to true, that turn's question is still shown to the "
    ...
    "- When every weak_or_missing and claims_to_verify item has been covered but more "
    "questions are still required, verify the strong matches instead: confirm a hard "
    "requirement still holds today (certification currency, availability for the exact "
    "shift, transportation, required equipment), or ask for a concrete example that "
    'backs up a resume claim. Target that item, e.g. "strong_matches:3".\n'
    ...
    f"- Set should_end to true once at least {MIN_QUESTIONS} questions have been asked "
    "AND every weak_or_missing and claims_to_verify item has received a clear, "
    "non-vague answer. Do not re-ask a gap that has been answered.\n"
