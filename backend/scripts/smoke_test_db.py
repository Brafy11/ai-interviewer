...
def main() -> None:
    ...
    with Session(engine) as session:
        ...
        jd = JobDescription(
            raw_text="Clearhaven Home Care is hiring a part-time caregiver in Fairview County, OH.",
            profile_json={"title": "Caregiver", "organization": "Clearhaven Home Care"},
        )
        session.add(resume)
        session.add(jd)
        session.commit()
    ...
    with Session(engine) as session:
        ...
        assert loaded_jd.profile_json["organization"] == "Clearhaven Home Care"
